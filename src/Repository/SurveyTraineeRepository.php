...
class SurveyTraineeRepository extends ServiceEntityRepository
{
    ...
    public function getGlobalSurveys(): array|bool
    {
        ...
        $query = $bdd->executeQuery(
            'SELECT st.*, a.*, u.* FROM survey_trainee st INNER JOIN user u INNER JOIN avatar a WHERE (u.id = st.trainee_id AND st.survey_id IS NOT NULL AND st.rate IS NOT NULL AND a.id = u.avatar_id) AND st.answer <> "";',
        );
