...
ini_set('memory_limit', '512M'); // Ajuste à la valeur désirée
...
class PdfGeneratorController extends AbstractController
{
    ...
    public function index(Request $request, ProspectRepository $prospectRepository, UserRepository $userRepository, TraineeRepository $traineeRepository, string $element)
    {
        ...
        // 'imageSrc'  => $this->imageToBase64($this->getParameter('kernel.project_dir') . '/avatars/default.jpeg'),
        ...
        //     $trainee = $traineeRepository->findOneBy(['username' => $userRepository->findOneBy(['username' => $request->attributes->get('trainee')])]);
    ...
    #[Route('/generate2/{element}/{trainee?}/{html?}', name: 'pdf_generate')]
    public function index2(Request $request, ProspectRepository $prospectRepository, UserRepository $userRepository, TraineeRepository $traineeRepository, string $element)
    {
        // To generate base64 image for displaying in PDF
        // 'imageSrc'  => $this->imageToBase64($this->getParameter('kernel.project_dir') . '/avatars/default.jpeg'),
        if ($this->isGranted('ROLE_TRAINER')) {
